...
/**
 * Codex CLI Adapter
 * Spawns the Codex CLI process to get responses
 */
export class CodexCliAdapter extends BaseAdapter {
  ...
  private getCliPath(): string {
    ...
    return (agent?.config.cliPath as string) || 'codex';
  }
...
- End with a confidence score: [CONFIDENCE: 0.X]`;
  ...
  private runCli(prompt: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const proc = spawn(cliPath, ['exec', '--skip-git-repo-check', quotedPrompt], {
        shell: true,
        env: { ...process.env },
        cwd: process.cwd(),
